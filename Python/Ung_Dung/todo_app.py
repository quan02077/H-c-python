...
class Tools:
    ...
    def add_save(self):
        self.task = self.entry_task.get().strip()
        self.priority_get = self.priority.get()
        self.deadline = self.deadline_task.get()

        self.today = datetime.now().strftime("%d/%m/%Y")
        self.d1 = datetime.strptime(self.deadline, "%d/%m/%Y")
        self.d2 = datetime.strptime(self.today, "%d/%m/%Y")

        if self.task == "" and self.priority == "" and self.d1 < self.d2:
            messagebox.showerror("Lỗi", "Vui lòng điền đầy đủ thông tin và kiểm tra lại ngày hết hạn!")
            return

        users = load_data()
        found = False

        tasks_data = {"task": self.task, "priority": self.priority_get, "deadline": self.deadline, "done": 0}
    
        for user in users:
            if user["info"] == self.username:
                for task in user.get("tasks", []):
                    if task.get("task") == self.task:
                        messagebox.showerror("Lỗi", f"Công việc {self.task} đã tồn tại!")
...
class Progress:
    ...
    def update_progress(self, var_clicked=None, deadline=None, task_name=None):
        ...
        if value >= 100:
            ...
            self.cong_label = tk.Label(self.frame_video, 
                                       text= "Chúc mừng bạn đã hoàn thành tất cả công việc!", 
                                       font=("Monospace", 14, "bold"),bg="#F1F3E0")
            ...
            self.frame_video.after(1050, self.player.play())
...
#     ToDoApp(root, "AnBaToCom", "qquan02077@gmail.com", "minhQuan2006")  # Tên người dùng giả lập
#     root.mainloop()
